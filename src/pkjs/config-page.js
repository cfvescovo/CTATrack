...
function buildConfigPageUrl() {
  ...
  var showTrainRunNumber = !!s.showTrainRunNumber;
  ...
  var html = [
    ...
    '.toggle{display:flex;gap:12px;align-items:flex-start;padding:14px 0 4px;}',
    '.toggle input{width:auto;margin:4px 0 0;transform:scale(1.15);}',
    '.toggle-copy{flex:1;}',
    '.toggle-copy label{margin:0 0 4px;}',
    ...
    '<div class="toggle">',
    '<input id="show-train-run-number" type="checkbox"', showTrainRunNumber ? ' checked' : '', '>',
    '<div class="toggle-copy">',
    '<label for="show-train-run-number">Show train run number</label>',
    '<div class="hint">Adds the CTA run number to each train arrival line when available.</div>',
    '</div>',
    '</div>',
    '<div class="actions">',
    ...
    'var showTrainRunNumberInput=document.getElementById("show-train-run-number");',
    ...
    'var marker="return_to=";',
    'var idx=href.lastIndexOf(marker);',
    'if(idx!==-1){',
    'returnTo=decodeURIComponent(href.substring(idx+marker.length));',
    '}',
    '}',
    'var body=encodeURIComponent(JSON.stringify(payload));',
    'window.location=returnTo?(returnTo+body):("pebblejs://close#"+body);',
    '}',
    ...
    'closeWith({distanceUnit:unitInput.value,themeMode:theme,trainRadius:train,busRadius:bus,showTrainRunNumber:showTrainRunNumberInput.checked});',
    '});',
